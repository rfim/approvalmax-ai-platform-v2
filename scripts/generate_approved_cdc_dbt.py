...
def render_model(context: str, business_key: str, bronze_table: str = "approvalmax_cdc_raw") -> str:
    ...
    if bronze_table == "pokemon_actor_raw":
        return f"""{{{{ config(
    materialized='table',
    alias='{model_name}',
    tags=['approved_cdc_context', 'generated', 'pokemon']
) }}}}

-- Generated after human approval of the `{context}` source context.
-- This model exposes a conservative current-state view from Pokemon Bronze only.
-- It does not define financial metrics or alter business keys.
with source_rows as (
    select
        {business_key},
        map('pokemon_key', pokemon_key) as primary_key,
        'pokemon_actor' as source_table,
        'pokeapi_pokemon' as cdc_schema,
        source_system,
        'upsert' as op,
        cast(pokemon_id as bigint) as sequence_id,
    ...
    from {{{{ source('bronze', 'pokemon_actor_raw') }}}}
),
